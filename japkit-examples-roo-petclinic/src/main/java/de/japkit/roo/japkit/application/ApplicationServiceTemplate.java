	...
	public abstract class ApplicationServiceMethodsForAggregate {
		...
		@CodeFragment(
			...
			cases = {
				...
				@Case(
					cond = "#{src.asType().asElement.ValueObject != null}",
					value = "new #{src.asType().code}.Builder()#{fluentVOSettersFromDTO()}.build()"),
				// TODO: Das ist Q&D, da es davon augeht, dass der App-Service
				// alle Aggregate bzw. ihre Repos kennt
				@Case(
					condFun = DomainLibrary.isEntity.class,
					value = "#{src.singleValueType.simpleName.toFirstLower}Repository.findById(command.#{findGetter(cmdProperties, src).simpleName}()).orElse(null)"), },
		 ...
		 * @Template(srcFun=entityProperties.class, srcGroupByFun={SrcSingleValueType.class,
		 *                                          findRepository.class} )
		 *                                          abstract class
		 *                                          RelatedEntityMembers{
		 * @Var(fun={SrcKey.class, nameFirstLower.class}) class
		 *                         repositoryFieldName {}
		 * @Field
		 * @Autowired private SrcKey $repositoryFieldName$; }
		 */
		 ...
		 *                  </pre>
		 */
		public Aggregate find$aggregateName$(long id, Long version) {
			return null;
		}
	}
